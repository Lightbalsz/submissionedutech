...
def data_preprocessing(input_array):
    numeric_part = np.array(input_array[:9], dtype=float).reshape(1, -1)
    categorical_part = map_input(*input_array[9:])

    # Hanya fitur numerik yang di-scale
    numeric_scaled = scaler.transform(numeric_part)

    # Gabungkan dengan fitur kategori
    full_data = np.concatenate([numeric_scaled, [categorical_part]], axis=1)

    df_scaled = pd.DataFrame(full_data, columns=[
